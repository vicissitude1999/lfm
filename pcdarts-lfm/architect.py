...
class Architect(object):
    ...
    def __init__(self, model, reweighted_model, model_beta, args):
    ...
    def _compute_reweighted_unrolled_model(self, input, target, weights, eta, network_optimizer):
        logits = self.reweighted_model(input)
        loss = F.cross_entropy(logits, target, reduction='none')
        loss = torch.dot(loss, weights)
        theta = _concat(self.reweighted_model.parameters()).data
        try:
            moment = _concat(network_optimizer.state[v]['momentum_buffer']
                             for v in self.reweighted_model.parameters()).mul_(self.network_momentum)
        except:
            moment = torch.zeros_like(theta)
        dtheta = _concat(torch.autograd.grad(
            loss, self.reweighted_model.parameters())).data + self.network_weight_decay * theta
    ...
    def _hessian_vector_product_rw(self, vector, input, target, weights, r=1e-2):
        R = r / _concat(vector).norm()
        # print(R)
        for p, v in zip(self.reweighted_model.parameters(), vector):
            p.data.add_(R, v)
        logits = self.reweighted_model(input)
        loss = F.cross_entropy(logits, target, reduction='none')
        loss = torch.dot(loss, weights)
        grads_p = torch.autograd.grad(
            loss, self.reweighted_model.arch_parameters())
